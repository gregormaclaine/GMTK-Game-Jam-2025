...
const JamLib = {
  ...
  Button: class {
    ...
    contains_mouse() {
      ...
      if (mouseX * JamLib.get_scale() < x - w / 2) return false;
      if (mouseX * JamLib.get_scale() > x + w / 2) return false;
      if (mouseY * JamLib.get_scale() < y - h / 2) return false;
      if (mouseY * JamLib.get_scale() > y + h / 2) return false;
      return true;
    }
  },
  ...
  Grid: class {
    ...
    show(data = []) {
      ...
      const active_cell = this.is_mouse_in_grid()
        ? this.what_cell(
            mouseX * JamLib.get_scale(),
            mouseY * JamLib.get_scale()
          )
        : [-1, -1];
    ...
    is_mouse_in_grid() {
      ...
      if (mouseX * JamLib.get_scale() < x - (w * rows) / 2) return false;
      if (mouseX * JamLib.get_scale() > x + (w * rows) / 2) return false;
      if (mouseY * JamLib.get_scale() < y - (h * cols) / 2) return false;
      if (mouseY * JamLib.get_scale() > y + (h * cols) / 2) return false;
      return true;
    }
